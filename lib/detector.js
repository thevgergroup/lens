...
const AI_SOFTWARE_STRINGS = [
  ...
  'Ideogram', 'Recraft', 'text-to-image', 'diffusion model',
];
...
function scanXmpString(xmpText) {
  ...
  // C2PA namespace declaration in XMP — only fires if the image has an embedded C2PA block
  // (not just a provenance URL that happens to contain "c2pa" as a substring)
  if (/xmlns:c2pa\s*=|<c2pa:/i.test(xmpText)) {
    signals.push({ type: 'c2pa', label: 'C2PA Content Credentials namespace in XMP', weight: 0.88 });
...
export function analyzePixelStatistics(imageData, isLossy = false) {
  ...
  // --- 3. Chroma warmth bias (YCbCr) ---
  // AI generators (Aurora/Grok, DALL-E, SD) exhibit a warm colour bias: more pixels
  // have stronger red/yellow chroma (|Cr-128| > |Cb-128|) than blue/cyan chroma.
  // Real photographs have roughly equal warm/cool pixel counts globally.
  //
  // Calibrated on 69 AI + 43 real photos:
  //   warmFrac > 0.50: AI 31/69 (45%), Real 6/43 (14%) — 84% precision
  //   Orthogonal to DCT signal — catches 10 AI images DCT misses entirely.
  let warmCount = 0;
  for (let i = 0; i < totalPixels; i++) {
    const cb = -0.168736 * r[i] - 0.331264 * g[i] + 0.5       * b[i]; // centred at 0
    const cr =  0.5      * r[i] - 0.418688 * g[i] - 0.081312  * b[i]; // centred at 0
    if (Math.abs(cr) > Math.abs(cb)) warmCount++;
  }
  const warmFrac = warmCount / totalPixels;
  if (warmFrac > 0.50) {
    const w = warmFrac > 0.70 ? 0.55 : 0.50;
    signals.push({ type: 'pixel', label: `Warm chroma bias: ${(warmFrac*100).toFixed(1)}% pixels warm-dominant`, weight: w });
    score = Math.max(score, w);
  }

  // --- 3b. Saturation distribution ---
  // AI images have a long tail of highly-saturated pixels from model colour bias.
  // Threshold calibrated on 69 AI + 43 real photos (browser canvas decode):
  //   > 0.20: AI 27/69 (39%), Real 8/43 (19%) — 77% precision
  // R-G and R-B channel correlation were removed: they fired on 51% / 47% of
  // real photos respectively (worse than random), driven by monochromatic scenes.
  let highSatCount = 0;
  for (let i = 0; i < totalPixels; i++) {
    const mx = Math.max(r[i], g[i], b[i]);
    const mn = Math.min(r[i], g[i], b[i]);
    if (mx > 10 && (mx - mn) / mx > 0.6) highSatCount++;
  }
  const highSatFrac = highSatCount / totalPixels;
  if (highSatFrac > 0.20) {
    const w = highSatFrac > 0.35 ? 0.60 : 0.50;
    signals.push({ type: 'pixel', label: `Elevated saturation: ${(highSatFrac*100).toFixed(1)}% pixels >0.6`, weight: w });
    score = Math.max(score, w);
  }
  ...
  // --- 5. DCT high-frequency energy ratio ---
  // AI generators (VQ-VAE, LDM decoder) concentrate image energy in low-frequency
  // DCT coefficients. Real photographs spread energy into mid/high AC coefficients
  // through natural texture and detail. JPEG compression preserves this ratio because
  // it operates in the DCT domain and does not equalise the distribution.
  //
  // Measure: fraction of AC energy in high-frequency coefficients (indices 28–63 of
  // the 64-element zig-zag order) across all 8×8 blocks in the image.
  //
  // Calibrated on 69 AI images + 43 real photos (CDN-stripped JPEGs included):
  //   AI mean: 0.134,  Real mean: 0.229  (nearly 2× separation)
  //   < 0.10 → 91% precision, 43% recall  (tight, near zero FP)
  //   < 0.12 → 88% precision, 54% recall  (broader, still high precision)
  if (totalPixels >= 10000) {
    const highFracResult = computeDCTHighFreqFraction(lum, width, height);
    if (highFracResult !== null) {
      const { meanHighFrac, blockCount } = highFracResult;
      if (meanHighFrac < 0.10 && blockCount >= 50) {
        signals.push({ type: 'pixel', label: `Low DCT high-freq energy: ${meanHighFrac.toFixed(3)} (AI decoder smoothing)`, weight: 0.50 });
        score = Math.max(score, 0.50);
      } else if (meanHighFrac < 0.12 && blockCount >= 50) {
        signals.push({ type: 'pixel', label: `Reduced DCT high-freq energy: ${meanHighFrac.toFixed(3)} (AI decoder smoothing)`, weight: 0.45 });
        score = Math.max(score, 0.45);
      }
    }
  }

  // --- 6. Laplace noise block correlation (Mallet et al. 2025) ---
  // Thresholds calibrated against browser-decoded pixel values (OffscreenCanvas/
  // ImageDecoder), not Node/sharp — the two decoders produce substantially different
  // values for JPEG/WebP due to chroma upsampling and color management differences.
  //
  // Browser-observed real image max: ant-photo hcf=0.456, picsum hcf=0.37-0.29
  // Browser-observed AI range: Aurora hcf=0.500-0.696, others hcf=0.18-0.38
  // Safe threshold hcf > 0.500 catches 5/6 Aurora images, zero false positives.
  // Aurora-vangogh-cat (hcf=0.376), SDXL, Bing, DALL-E all overlap with real photos.
  if (totalPixels >= 16384) {
    const noiseCorr = computeBlockNoiseCorrelation(r, g, b, width, height);
    if (noiseCorr !== null) {
      const { highCorrFrac, absMean } = noiseCorr;
      if (highCorrFrac > 0.500) {
        const excess = highCorrFrac / 0.500 - 1;
        const weight = excess > 0.3 ? 0.52 : 0.45;
        signals.push({
          type: 'pixel',
...
/**
 * Measures the fraction of DCT AC energy in high-frequency coefficients (indices 28–63)
 * across all 8×8 blocks of the luminance channel.
 *
 * AI generator decoders (VQ-VAE, LDM) produce images whose energy is concentrated
 * in low-frequency DCT coefficients. Real photographs spread energy into high-frequency
 * coefficients through natural texture and sensor detail.
 *
 * Returns { meanHighFrac, blockCount } or null if too few blocks.
 */
function computeDCTHighFreqFraction(lum, width, height) {
  // 1D DCT-II on 8 values
  function dct8(v, out) {
    for (let k = 0; k < 8; k++) {
      let s = 0;
  ...
  const dctOut = new Float32Array(8);
  ...
  for (let by = 0; by + 8 <= height; by += 8) {
    for (let bx = 0; bx + 8 <= width; bx += 8) {
      // Fill block, centre around 128
      for (let r = 0; r < 8; r++)
        for (let c = 0; c < 8; c++)
          tmp[r * 8 + c] = lum[(by + r) * width + (bx + c)] - 128;

      // Row DCT
      for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) row[c] = tmp[r * 8 + c];
        dct8(row, dctOut);
        for (let c = 0; c < 8; c++) tmp[r * 8 + c] = dctOut[c];
      }
      ...
      for (let c = 0; c < 8; c++) {
        ...
        dct8(col, dctOut);
        for (let r = 0; r < 8; r++) tmp[r * 8 + c] = dctOut[r];
      }

      // Measure AC energy split: low (coefficients 1–27) vs high (28–63)
      // DC coefficient at [0] is excluded.
      let acLow = 0, acHigh = 0;
      for (let k = 1; k < 64; k++) {
        const e = tmp[k] * tmp[k];
        if (k < 28) acLow += e;
        else acHigh += e;
      }
      const acTotal = acLow + acHigh;
      if (acTotal > 0) {
        highFracSum += acHigh / acTotal;
        blockCount++;
      }
    }
  }

  if (blockCount < 10) return null;
  return { meanHighFrac: highFracSum / blockCount, blockCount };
}
