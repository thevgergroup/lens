...
function scanJumbfBox(data) {
  ...
  const hasC2pa = text.includes('c2pa');
  if (!hasC2pa) return { score, signals };

  // At this point we know it's a C2PA manifest
  let label = 'C2PA Content Credentials (JUMBF)';
  let weight = 0.88;
 ...
 */
export function analyzePixelStatistics(imageData) {
  ...
  // AI images: structured LSBs from quantisation or watermarking → lower entropy
  const lsbCounts = [0, 0];
  for (let i = 0; i < totalPixels; i++) {
  ...
  if (avgNoise < 1.2 && totalPixels > 50000) {
    signals.push({ type: 'pixel', label: `Very low noise floor: ${avgNoise.toFixed(3)}`, weight: 0.5 });
    score = Math.max(score, 0.5);
  }

  // --- 5. SDXL edge chrominance fringing ---
  // SDXL 1.0 VAE produces green/violet color artifacts along high-contrast luminance edges.
  // Measured as: at pixels where |∇lum| is large, check if G-R or B-R diverges unusually.
  // Reference: https://hforsten.com/identifying-stable-diffusion-xl-10-images-from-vae-artifacts.html
  if (totalPixels >= 65536) { // Only meaningful on images ≥256x256
    let edgeChromSum = 0;
    let edgeSamples = 0;
    const edgeStep = Math.max(1, Math.floor(Math.sqrt(totalPixels / 8000)));
    ...
    for (let y = 1; y < height - 1; y += edgeStep) {
      for (let x = 1; x < width - 1; x += edgeStep) {
        ...
        if (gradMag > 20) { // Only at actual edges
          // Measure chrominance deviation: how much does G channel differ from luminance prediction?
          const predicted_g = lum[i]; // lum ≈ 0.299R + 0.587G + 0.114B, so G ≈ lum for neutral
          const chromDevG = Math.abs(g[i] - predicted_g);
          const chromDevB = Math.abs(b[i] - predicted_g);
          edgeChromSum += Math.max(chromDevG, chromDevB) / gradMag;
          edgeSamples++;
        }
      }
    }
    ...
    if (edgeSamples > 50) {
      const edgeChromRatio = edgeChromSum / edgeSamples;
      if (edgeChromRatio > 0.35) {
        signals.push({ type: 'pixel', label: `Edge chrominance fringing: ${edgeChromRatio.toFixed(3)} (SDXL VAE artifact)`, weight: 0.55 });
        score = Math.max(score, 0.55);
      }
    }
  }
...
export function analyzeFrequencyDomain(imageData) {
  ...
  // --- HF energy attenuation (universal latent diffusion fingerprint) ---
  // LDM VAE bottleneck filters stochastic high-frequency sensor noise.
  // Real camera photos: HF energy / total energy ratio typically 0.08–0.25.
  // FLUX/SDXL/SD outputs: ratio typically 0.03–0.07 (VAE smoothing).
  // Reference: arXiv 2602.00192 (Theorem 3.2)
  const { hfRatio } = measureHfAttenuation(real, imag, fftSize);
  if (hfRatio < 0.055) {
    const weight = hfRatio < 0.035 ? 0.60 : 0.45;
    signals.push({
      type: 'fft',
      label: `Low HF energy ratio: ${hfRatio.toFixed(4)} (VAE smoothing — LDM fingerprint)`,
      weight,
    });
    score = Math.max(score, weight);
  }

  // --- Synthbuster cross-difference periodicity ---
  // VAE decoder uses transposed convolutions with stride 2 (upsampling ×8).
  // This introduces periodic artifacts at spatial frequencies N/8, N/4, N/2.
  // Real images have no such periodicity in their noise residual.
  // Reference: Bammey 2023 (IEEE TIFS) — https://github.com/qbammey/synthbuster
  const periodicityScore = measureUpsamplingPeriodicity(noise, fftSize);
  if (periodicityScore > 0.40) {
    const weight = Math.min(0.65, periodicityScore * 0.8);
    signals.push({
      type: 'fft',
      label: `Upsampling periodicity: ${periodicityScore.toFixed(3)} (VAE decoder stride artifact)`,
      weight,
    });
    score = Math.max(score, weight);
  }
...
/**
 * Measures ratio of high-frequency energy to total energy in the FFT magnitude spectrum.
 * LDM VAEs attenuate stochastic HF noise, producing a distinctively low HF/total ratio.
 * Computed on the noise residual FFT (already high-pass filtered).
 */
function measureHfAttenuation(real, imag, size) {
  const half = size / 2;
  let totalEnergy = 0;
  let hfEnergy = 0;

  // HF region: spatial frequencies in outer 40% of spectrum (radius > 0.6 * Nyquist)
  const hfRadiusSq = (half * 0.6) ** 2;

  for (let fy = 0; fy < size; fy++) {
    for (let fx = 0; fx < size; fx++) {
      const idx = fy * size + fx;
      const mag2 = real[idx] * real[idx] + imag[idx] * imag[idx];
      totalEnergy += mag2;

      // Shift origin to centre for radius calculation
      const cx = fx < half ? fx : fx - size;
      const cy = fy < half ? fy : fy - size;
      if (cx*cx + cy*cy > hfRadiusSq) hfEnergy += mag2;
    }
  }
...
/**
 * Detects periodic peaks in the FFT magnitude at spatial frequencies corresponding
 * to VAE decoder stride-2 upsampling (N/8, N/4, N/2 harmonics).
 * Inspired by Synthbuster (Bammey 2023) cross-difference filter approach.
 */
function measureUpsamplingPeriodicity(noise, size) {
  // Apply cross-difference filter: f(x,y) - f(x+1,y) + f(x,y) - f(x,y+1)
  // This amplifies the periodic upsampling grid artifact
  const filtered = new Float32Array(size * size);
  for (let y = 0; y < size - 1; y++) {
    for (let x = 0; x < size - 1; x++) {
      const i = y * size + x;
      filtered[i] = noise[i] - noise[i + 1] + noise[i] - noise[i + size];
    }
  }

  const { real: fr, imag: fi } = fft2d(filtered, size);

  // Check energy at expected upsampling harmonics: size/8, size/4, size/2
  const harmonics = [Math.round(size/8), Math.round(size/4), Math.round(size/2)];
  let harmonicEnergy = 0;
  let totalMagEnergy = 0;
  const bandWidth = 2; // ±2 bins around each harmonic

  for (let fy = 0; fy < size; fy++) {
    for (let fx = 0; fx < size; fx++) {
      const idx = fy * size + fx;
      const mag2 = fr[idx]*fr[idx] + fi[idx]*fi[idx];
      totalMagEnergy += mag2;

      const atHarmonic = harmonics.some(h =>
        (Math.abs(fx - h) <= bandWidth || Math.abs(fx - (size - h)) <= bandWidth) &&
        (Math.abs(fy - h) <= bandWidth || Math.abs(fy - (size - h)) <= bandWidth)
      );
      if (atHarmonic) harmonicEnergy += mag2;
    }
  }

  // Normalise by expected proportion of spectrum covered by harmonic bands
  const bandArea = harmonics.length * (2 * bandWidth + 1) ** 2 * 4; // 4 quadrant symmetry
  const expectedFraction = bandArea / (size * size);
  const observed = totalMagEnergy > 0 ? (harmonicEnergy / totalMagEnergy) : 0;
  const periodicityScore = Math.max(0, (observed / expectedFraction) - 1) / 10; // normalise to ~0..1

  return Math.min(1, periodicityScore);
}
