...
export function analyzePixelStatistics(imageData, isLossy = false) {
  ...
  // --- 2. Local gradient smoothness (corroborating) ---
  // AI images: unusually smooth gradients with sharp semantic edges
  // Measure: average magnitude of 3x3 Sobel gradient
  // Individual precision ~80% — too many smooth-sky real photos. Counts toward
  // corroboration gate below; does not flag alone.
  let gradSum = 0;
  ...
  // Collect corroborating weak signals — each individually has ~80-85% precision,
  // which is not enough to flag alone. Two or more agreeing is compelling.
  const corrobSignals = [];

  if (avgGradient < 8 && totalPixels > 10000) {
    corrobSignals.push({ type: 'pixel', label: `Unusually smooth gradient: ${avgGradient.toFixed(2)}` });
  }

  // --- 3. Chroma warmth bias (YCbCr) (corroborating) ---
  // AI generators (Aurora/Grok, DALL-E, SD) exhibit a warm colour bias: more pixels
  // have stronger red/yellow chroma (|Cr| > |Cb|) than blue/cyan chroma.
  //
  // Calibrated on 69 AI + 43 real photos:
  //   warmFrac > 0.50: AI 31/69 (45%), Real 6/43 (14%) — 84% precision alone
  //   Orthogonal to DCT — catches AI images DCT misses entirely.
  let warmCount = 0;
  for (let i = 0; i < totalPixels; i++) {
    const cb = -0.168736 * r[i] - 0.331264 * g[i] + 0.5      * b[i];
    const cr =  0.5      * r[i] - 0.418688 * g[i] - 0.081312 * b[i];
    if (Math.abs(cr) > Math.abs(cb)) warmCount++;
  }
  const warmFrac = warmCount / totalPixels;
  if (warmFrac > 0.50) {
    corrobSignals.push({ type: 'pixel', label: `Warm chroma bias: ${(warmFrac*100).toFixed(1)}% pixels warm-dominant` });
  }

  // --- 3b. Saturation distribution (corroborating) ---
  ...
  //   > 0.20: AI 27/69 (39%), Real 8/43 (19%) — 77% precision alone
  let highSatCount = 0;
  for (let i = 0; i < totalPixels; i++) {
  ...
  if (highSatFrac > 0.20) {
    corrobSignals.push({ type: 'pixel', label: `Elevated saturation: ${(highSatFrac*100).toFixed(1)}% pixels >0.6` });
  }

  // --- 4. Noise floor analysis (standalone, PNG only) ---
  // Compute local noise as difference between pixel and blurred neighbourhood.
  // Skip lossy — JPEG DCT smoothing produces artificially low local noise in real photos.
  // PNG real images have sensor noise → high avgNoise; AI PNGs are synthetically smooth.
  let noiseSum = 0;
  let noiseSamples = 0;
  ...
  // --- Corroboration gate ---
  // 1 weak signal alone → score 0.35 (below "possible" threshold, shows in UI as unlikely)
  // 2+ weak signals agreeing → score 0.50 (possible — two independent signals converging)
  // 3 weak signals → score 0.55 (stronger convergence)
  if (corrobSignals.length === 1) {
    const sig = { ...corrobSignals[0], weight: 0.35 };
    signals.push(sig);
    score = Math.max(score, 0.35);
  } else if (corrobSignals.length === 2) {
    for (const s of corrobSignals) signals.push({ ...s, weight: 0.50 });
    score = Math.max(score, 0.50);
  } else if (corrobSignals.length >= 3) {
    for (const s of corrobSignals) signals.push({ ...s, weight: 0.55 });
    score = Math.max(score, 0.55);
  }
