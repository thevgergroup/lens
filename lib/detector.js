...
export function analyzePixelStatistics(imageData, isLossy = false) {
  ...
  // AI models produce images with unnaturally high inter-channel correlation —
  // particularly R-B which is decorrelated in natural scenes with varied lighting.
  ...
  let covRG = 0, covRB = 0, varR = 0, varG = 0, varB = 0;
  for (let i = 0; i < totalPixels; i++) {
    const dr = r[i] - meanR, dg = g[i] - meanG, db = b[i] - meanB;
    covRG += dr * dg; covRB += dr * db;
  ...
  // R-G correlation: natural photos rarely exceed 0.97 across whole image
  if (Math.abs(corrRG) > 0.97) {
    signals.push({ type: 'pixel', label: `High R-G correlation: ${corrRG.toFixed(3)}`, weight: 0.50 });
    score = Math.max(score, 0.50);
  }
  // R-B correlation: strong discriminator — natural scenes have varied R-B relationship
  if (Math.abs(corrRB) > 0.95) {
    signals.push({ type: 'pixel', label: `High R-B correlation: ${corrRB.toFixed(3)}`, weight: 0.55 });
    score = Math.max(score, 0.55);
  }

  // --- 3b. Saturation distribution ---
  // AI images have a long tail of highly-saturated pixels from model color bias.
  // Natural photos rarely exceed 5% of pixels above 0.6 HSV saturation.
  let highSatCount = 0;
  for (let i = 0; i < totalPixels; i++) {
  ...
  if (highSatFrac > 0.08) {
    const w = highSatFrac > 0.20 ? 0.60 : 0.50;
